    ...
    try {
      ...
      const html = normalized.map(rr => `
        <div style="padding:10px; border-bottom:1px solid rgba(255,255,255,0.03);">
          <div style="font-weight:700">Q: ${rr.QID || rr.qid} <span style="color:#94a3b8; font-weight:600;">(${rr.Section || rr.section || 'General'})</span></div>
          <div style="color:#94a3b8;">${rr.Question || rr.question || ''}</div>
          <div style="margin-top:6px;">Selected: <strong style="color:#f87171">${rr.SelectedAnswer || rr.selectedAnswer || '—'}</strong> | Correct: <strong style="color:#4ade80">${rr.CorrectAnswer || rr.correctAnswer || '—'}</strong></div>
        </div>
      `).join('');
      ...
      if (w && w.document) {
        ...
        const detailsModal = document.getElementById('detailsModal');
        if (detailsModalBody && detailsModal) {
          detailsModalBody.innerHTML = renderScoreDeductionSummary(record) + html;
          detailsModal.style.display = 'flex';
        } else {
          alert('Response Details:\n' + normalized.length + ' responses fetched.');
        }
      }
    } catch (e) {
