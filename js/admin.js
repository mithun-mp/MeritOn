 ...
 * @param {Set<string>} existingIds - Set of existing QID strings to avoid.
 * @returns {string} A unique QID in the format 'Q<number>'.
 */
function generateUniqueManagerQid(existingIds) {
    let maxNum = 0;
...
async function saveAllManagerChanges() {
    ...
    try {
        ...
        // Build a set of existing QIDs from originalManagerQuestions (using the QID field, which is uppercase in the objects we have)
        const existingIds = new Set();
        for (const q of originalManagerQuestions) {
            // We assume the QID is stored in the QID property (uppercase) as per the frontend usage
            const id = q.QID;
            if (id) {
        ...
        // Generate new questions with unique QIDs
        const newQuestions = [];
        const newIdsSet = new Set(); // to track QIDs we've assigned in this batch
        for (const q of currentManagerQuestions) {
            if (q.isNew === true) {
                // Generate a unique QID that does not exist in existingIds and not in newIdsSet
                let qid;
                do {
                    qid = generateUniqueManagerQid(new Set([...existingIds, ...newIdsSet]));
                } while (newIdsSet.has(qid)); // though the function above should avoid duplicates, we double-check
                newIdsSet.add(qid);

                newQuestions.push({
                    qid: qid, // note: sending lowercase qid for the backend's addQuestions
                    ...
                    negativeMarks: q.negativeMarks || q.NegativeMarks
                });
            }
        }
        ...
        if (modifiedExisting.length > 0) {
            const updates = modifiedExisting.map(q => ({
                qid: q.QID, // note: sending uppercase QID, but backend's getQuestionId handles both cases
                updatedData: {
                    question: q.Question,
                    section: q.Section,
                    correct: q.Correct,
