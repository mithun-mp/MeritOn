...
function restoreFromSession() {
    ...
    try {
        ...
        // Authoritative server-aligned remaining time calculation (SEC-008)
        if (startedAt && testData && testData.Duration) {
            const elapsedSeconds = Math.floor((Date.now() - new Date(startedAt).getTime()) / 1000);
            const totalDurationSeconds = (Number(testData.Duration) || 60) * 60;
            timeLeft = Math.max(0, totalDurationSeconds - elapsedSeconds);
            debugLog('INFO', 'RECOVERY', `Authoritative timer restored: ${timeLeft}s remaining based on startedAt ${startedAt}`);
        } else {
...
async function initExam(testId) {
    try {
        ...
        if (instAvatar) instAvatar.src = window.getAvatarPath ? window.getAvatarPath(user.avatar) : `assets/avatars/avatar${(user.avatar !== undefined && user.avatar !== null) ? user.avatar : 1}.png`;
...
async function submitExam() {
    ...
    try {
        ...
        if (res.success) {
            // BLOCKER-002: Handle queued submissions with polling instead of premature redirect
            if (res.queued) {
                const loaderTextEl = submitLoader ? submitLoader.querySelector('.loader-text-erit, .loader-subtext, p, span') : null;
                if (loaderTextEl) {
                    loaderTextEl.innerText = 'Submission processing...';
                }

                const queueId = res.queueId;
                const pollIntervalMs = 2500;
                const maxAttempts = 30; // 75 seconds maximum
                let attempts = 0;
                ...
                while (attempts < maxAttempts && !isCompleted) {
                    ...
                    try {
                        const statusRes = await api.get('getSubmissionStatus', { queueId });
                        if (statusRes.success) {
                            if (statusRes.status === 'completed') {
                                isCompleted = true;
                                pollResult = statusRes;
                            } else if (statusRes.status === 'failed') {
                                throw new Error(statusRes.error || 'Submission processing failed on background worker.');
                            }
                            // Continue polling while status is 'pending' or 'processing'
                        } else {
                            console.warn('[QUEUE POLL] Status query unsuccessful:', statusRes.error);
                        }
                    } catch (pollErr) {
                        if (pollErr.message && pollErr.message.includes('failed on background worker')) {
                            throw pollErr;
                        }
                        console.warn('[QUEUE POLL] Temporary network error while polling status:', pollErr);
                    }
                }

                if (!isCompleted) {
                    throw new Error('Submission is still being processed by the worker. Please check the lobby in a moment to view your result.');
                }
            ...
            // Direct (synchronous) submission success
            submissionComplete = true;
            localStorage.removeItem(getSessionKey());
            localStorage.setItem('lastResult', JSON.stringify({
                ...res,
...
function startTimer() {
    ...
    const update = () => {
        // Continuous wall-clock elapsed time calculation from startedAt (EXAM-001)
        if (startedAt && testData && testData.Duration) {
            const elapsed = Math.floor((Date.now() - new Date(startedAt).getTime()) / 1000);
            const totalDuration = (Number(testData.Duration) || 60) * 60;
    ...
    try {
        ...
        if (!liveExamSessionId) {
            ...
            if (sessionRes.success) {
                ...
                if (sessionRes.startedAt) {
                    startedAt = new Date(sessionRes.startedAt).toISOString();
                }
                saveToSession();
