...
window.formatTargetPill = function(target, college = '') {
    if (!target || typeof target !== 'object') return '🌍 Open to Everyone';
    const dept = (target.department || target.Department || '').trim();
    const year = (target.year || target.Year || '').trim();
    const batch = (target.batch || target.Batch || '').trim();
    ...
    else if (batch) parts.push(`Batch ${batch}`);
...
function displayUserInfo() {
    ...
    const avatarSrc = window.getAvatarPath ? window.getAvatarPath(user.avatar) : `assets/avatars/avatar${(user.avatar !== undefined && user.avatar !== null) ? user.avatar : 1}.png`;
...
async function syncSessionProfileOnce() {
    const user = getUser();
    if (!user || !user.sessionToken) return;

    try {
        const res = await api.post({ action: 'getProfile', sessionToken: user.sessionToken });
        if (res && res.success && res.user) {
            ...
            const currentYear = user.year || user.Year || '';
            const currentPhone = user.phone || user.Phone || '';
            const currentCollege = user.college || user.College || '';

            const isDifferent = (fresh.avatar !== undefined && fresh.avatar !== currentAvatar) ||
                                (fresh.fullName && fresh.fullName !== currentFullName) ||
                                (fresh.department && fresh.department !== currentDept) ||
                                (fresh.year && fresh.year !== currentYear) ||
                                (fresh.phone && fresh.phone !== currentPhone) ||
                                (fresh.college && fresh.college !== currentCollege);
            ...
            if (isDifferent) {
                ...
                if (typeof debugLog === 'function') {
                    debugLog('INFO', 'SESSION_SYNC', 'Session synced cleanly with MongoDB changes');
                }
            }
        }
    } catch (e) {
        // Silent catch: network glitch should not disrupt candidate experience
    }
}
...
window.showProfileUpdate = function() {
    ...
    if (selectedAvatar < 0 || selectedAvatar > 11) selectedAvatar = 1;
...
// Optimization: Refresh data immediately upon tab refocus if the user was on another tab
if (typeof document !== 'undefined') {
    document.addEventListener('visibilitychange', () => {
        if (!document.hidden) {
            loadCandidateData('tab-refocus');
            loadOverallLeaderboard('tab-refocus');
        }
    });
}
...
function createCandidateTestCard(test, status, isNew = true) {
    ...
    div.innerHTML = `
        ...
        <h3>${test.Name}</h3>
        <div style="font-size:0.78rem; color:#60a5fa; margin-bottom:12px; font-weight:600;">
            ${window.formatTargetPill ? window.formatTargetPill(test.Target || test.target || test.meta?.target, (getUser() || {}).college || 'APJ Abdul Kalam Technological University') : '🌍 Open to Everyone'}
        </div>
        <div class="test-meta">
            <div class="meta-item">
            ...
            <tr ${rowClass}>
                ...
                <td style="padding:12px 15px;">
                    <div style="display:flex; align-items:center; gap:10px;">
                        <img src="${window.getAvatarPath ? window.getAvatarPath(entry.avatar) : `assets/avatars/avatar${entry.avatar !== undefined && entry.avatar !== null ? entry.avatar : 1}.png`}" class="cbt-avatar avatar-40" alt="Candidate Avatar">
