/**
 * Test Lobby Logic - Phase 20 Upgrade
 */
...
window.openLiveLeaderboard = async function(testId, testName) {
  ...
  const modalHtml = `
      <div id="liveLeaderboardModal" class="modal-overlay" style="position: fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.85); backdrop-filter:blur(10px); z-index: 1000; display:flex; align-items:center; justify-content:center; padding:20px;">
          <div class="modal-content" style="background:#1e293b; border:1px solid rgba(255,255,255,0.1); border-radius:28px; padding:35px; width:100%; max-width:1200px; box-shadow:0 25px 50px rgba(0,0,0,0.5); max-height:90vh; overflow:auto;">
              <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:25px;">
...
function renderLiveLeaderboard(data, currentUserId) {
  ...
  if (leaderboard.length === 0) {
    container.innerHTML = `
          <div style="text-align:center; padding:60px 20px;">
              ...
              <p style="color:#94a3b8; margin-top:8px;">Leaderboard updates automatically as submissions come in.</p>
          </div>
      `;
    return;
  }
  ...
  const rowsWithAnimations = leaderboard.map(entry => {
    ...
    let animationClass = '';
    if (!previous) {
      animationClass = 'lb-new-row';
    } else if (entry.rank < previous.rank) {
      animationClass = 'lb-rank-up';
    } else if (entry.rank > previous.rank) {
      animationClass = 'lb-rank-down';
    } else if (
      entry.scorePercentile !== previous.scorePercentile ||
      entry.netScore !== previous.netScore ||
      entry.totalTimeTakenSeconds !== previous.totalTimeTakenSeconds
    ) {
      animationClass = 'lb-updated-row';
    }
  ...
  leaderboard.forEach(entry => {
    liveLeaderboardPreviousState.set(entry.userID, {
      ...
      totalTimeTakenSeconds: entry.totalTimeTakenSeconds
    });
  });
              ...
              <table style="width:100%; border-collapse:collapse;">
                  <thead>
                      <tr style="border-bottom:1px solid rgba(255,255,255,0.1);">
                          ...
                          <th style="text-align:left; padding:12px 15px; color:#94a3b8; font-size:0.85rem;">Correct</th>
                          <th style="text-align:left; padding:12px 15px; color:#94a3b8; font-size:0.85rem;">Wrong</th>
                          <th style="text-align:left; padding:12px 15px; color:#94a3b8; font-size:0.85rem;">Unanswered</th>
                          <th style="text-align:left; padding:12px 15px; color:#94a3b8; font-size:0.85rem;">Submitted At</th>
                      </tr>
                  </thead>
                          ...
                          return `
                              <tr data-user-id="${entry.userID}" data-rank="${entry.rank}" class="${entry.animationClass}" style="${rowClass} border-bottom:1px solid rgba(255,255,255,0.05);">
                                  <td style="padding:12px 15px;">
                                      <strong>#${entry.rank}${entry.isCurrentUser ? ' <span class="badge" style="background:#10b981; color:white; font-size:0.7rem; padding:2px 6px; border-radius:10px;">You</span>' : ''}</strong>
                                  </td>
                                  <td style="padding:12px 15px;"><strong>${entry.name}</strong></td>
                                  ...
                                  <td style="padding:12px 15px;">${formatTimeMMSS(entry.totalTimeTakenSeconds, "seconds")}</td>
                                  <td style="padding:12px 15px;">${entry.correctCount}</td>
                                  <td style="padding:12px 15px;">${entry.wrongCount}</td>
                                  <td style="padding:12px 15px;">${entry.unansweredCount}</td>
                                  <td style="padding:12px 15px;">${entry.submittedAt ? new Date(entry.submittedAt).toLocaleString() : ''}</td>
                              </tr>
                          `;
                      }).join('')}
                  </tbody>
  ...
  setTimeout(() => {
    ...
    animatedRows.forEach(row => {
      row.classList.remove('lb-new-row', 'lb-rank-up', 'lb-rank-down', 'lb-updated-row');
    });
  }, 1500);
}
