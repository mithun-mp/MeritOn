...
function normalizePdfText(value) {
    ...
    // Fix UTF-8 / Windows-1252 double-encoding artifacts
    const artifactMap = {
        'â€™': "'",
        'â€˜': "'",
        'â€“': "-",
...
async function generateQuestionPaper(result) {
    ...
    try {
        ...
        const [tests, questions, responses] = await Promise.all([
            ...
            api.get('getResponses', { testId, userID: userId })
        ]);
        ...
        const responseMap = {};
        const respList = Array.isArray(responses) 
            ? responses 
            : (responses?.Responses || responses?.data || responses?.responses || []);
        
        respList.forEach(r => {
            const qid = String(r.QID || r.qid || r.questionId || '').trim();
            if (qid) {
                responseMap[qid] = r;
                responseMap[qid.toLowerCase()] = r;
                responseMap[qid.toUpperCase()] = r;
            }
        });
        ...
        Object.keys(grouped).forEach(sectionName => {
            ...
            grouped[sectionName].forEach(q => {
                ...
                const qidKey = String(q.QID || q.qid || q.id || '').trim();
                const userResponse = responseMap[qidKey] || responseMap[qidKey.toLowerCase()] || responseMap[qidKey.toUpperCase()];
                const correctAnswerRaw = String(userResponse?.Correct || q.Correct || q.correct || '').trim().toUpperCase();
                const correctAnswer = ['A', 'B', 'C', 'D'].includes(correctAnswerRaw) ? correctAnswerRaw : 'N/A';
                const selectedAnswerRaw = userResponse ? String(userResponse.SelectedAnswer || userResponse.selectedAnswer || userResponse.selected || '').trim().toUpperCase() : '';
