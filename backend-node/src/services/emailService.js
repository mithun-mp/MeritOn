...
// Maintenance mode helper - configuration-driven (default false)
function isMailMaintenanceMode() {
  return process.env.MAIL_MAINTENANCE_MODE === 'true';
}
...
async function sendEmail({ to, subject, html, text }) {
  if (isMailMaintenanceMode()) {
    console.log(`[SMTP] Mail delivery skipped (MAIL_MAINTENANCE_MODE=true) for ${to}`);
    return {
      success: true,
      skipped: true,
      mailStatus: "maintenance_mode",
      message: "Email delivery is currently in maintenance mode."
    };
  }
  if (!smtpConfigured) {
    if (isDev) {
