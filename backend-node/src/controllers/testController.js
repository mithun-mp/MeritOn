  ...
  try {
    ...
    const processedTests = tests.map(testObj => {
      ...
      return {
        ...
        status,
        canLogin,
        StartTime: testObj.StartTime || startStr,
        ExpiryTime: testObj.ExpiryTime || testObj.EndTime || expiryStr,
        StartTimeDisplay: testObj.StartTimeDisplay || startDisplay,
        ExpiryTimeDisplay: testObj.ExpiryTimeDisplay || expiryDisplay,
        EndTime: testObj.EndTime || testObj.ExpiryTime || endStr,
        Date: testObj.Date || (startAt ? startAt.toISOString().split('T')[0] : ''),
...
async function createTest(testData, sessionToken) {
    try {
        ...
        const cleanDateStr = typeof testData.date === 'string' ? testData.date.split('T')[0] : testData.date;
        const cleanDateObj = new Date(`${cleanDateStr}T00:00:00.000Z`);
    ...
    try {
        ...
        if (mode === testPaperUtils.STORAGE_MODES.LEGACY || mode === testPaperUtils.STORAGE_MODES.DUAL) {
            ...
            if (test) {
                ...
                if (updatedData.date) {
                    const cleanDateStr = typeof updatedData.date === 'string' ? updatedData.date.split('T')[0] : updatedData.date;
                    test.Date = new Date(`${cleanDateStr}T00:00:00.000Z`);
                }
                if (updatedData.target) test.Target = updatedData.target;
        ...
        if (mode === testPaperUtils.STORAGE_MODES.DUAL || mode === testPaperUtils.STORAGE_MODES.OPTIMIZED) {
            ...
            if (testPaper) {
                ...
                if (updatedData.date) {
                    const cleanDateStr = typeof updatedData.date === 'string' ? updatedData.date.split('T')[0] : updatedData.date;
                    testPaper.meta.date = new Date(`${cleanDateStr}T00:00:00.000Z`);
                }
