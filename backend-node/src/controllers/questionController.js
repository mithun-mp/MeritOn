...
async function addQuestions(testId, questions, sessionToken) {
  try {
    ...
    const normalizedIncoming = questions.map((qItem) => {
      const q = qItem.updatedData ? { qid: qItem.qid, ...qItem.updatedData } : qItem;
...
async function bulkUpdateQuestions(testId, questions, sessionToken) {
  // Support both object payload format and positional parameters
  if (typeof testId === 'object' && testId !== null) {
    const payload = testId;
    sessionToken = payload.sessionToken || sessionToken;
    questions = payload.updates || payload.questions || questions;
  ...
  try {
    ...
    if (mode === testPaperUtils.STORAGE_MODES.DUAL || mode === testPaperUtils.STORAGE_MODES.OPTIMIZED) {
      ...
      const sectionNames = Array.from(new Set([
        ...(testPaper.sectionNames || []).map(s => typeof s === 'string' ? s : s.name),
    ...
    if (mode === testPaperUtils.STORAGE_MODES.DUAL || mode === testPaperUtils.STORAGE_MODES.OPTIMIZED || mode === testPaperUtils.STORAGE_MODES.LEGACY) {
      ...
      if (mode === testPaperUtils.STORAGE_MODES.DUAL || mode === testPaperUtils.STORAGE_MODES.OPTIMIZED) {
        ...
        legacyQuestions = testPaper.questions.map(q => ({
          TestID: testId,
          Section: q.section,
          QID: q.qid || q.questionId || String(q._id),
          Difficulty: q.difficulty,
          Question: q.question,
          A: q.options?.A || '',
          B: q.options?.B || '',
    ...
    console.error("[bulkUpdateQuestions CRITICAL ERROR]", {
      message: err.message,
      name: err.name,
      stack: err.stack
    });
...
async function uploadQuestionImage(req) {
  try {
    ...
    // Accept any valid dimensions returned by Cloudinary (allowing rectangular diagrams, charts, formulas, etc.)
