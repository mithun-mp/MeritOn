...
let uriHostname = 'configured';
try {
  const uriUrl = new URL(srvUri);
  uriHostname = uriUrl.hostname;
} catch (e) {}
console.log('3. URI Hostname:', uriHostname);
console.log('4. MongoDB URI configured: yes');
  ...
  shards.forEach((shard, idx) => {
    dns.lookup(shard, (err, address) => {
      ...
      if (idx === shards.length - 1) {
        ...
        if (process.env.MONGODB_STANDARD_URI) {
          setTimeout(() => {
            console.log('\n8. Testing Standard URI Connection:');
            testConnection(process.env.MONGODB_STANDARD_URI, 'Standard');
          }, 2000);
        }
      }
    });
